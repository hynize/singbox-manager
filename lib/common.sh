...
stop_argo_node() {
  local tag="$1"
  kill_pid_file "${RUNTIME_DIR}/${tag}.pid"
}
...
start_argo_temp_tunnel() {
  ...
  if domain="$(wait_for_trycloudflare_domain "${log_file}" 60 2)"; then
    ...
    local tunnel_pid
    tunnel_pid="$(read_pid_file "${pid_file}" 2>/dev/null || true)"
...
start_argo_node() {
  ...
  [ -x "${CLOUDFLARED_BIN}" ] || return 1
  mode="$(node_value "$tag" "argo_mode")"

  stop_argo_node "$tag"

  if [ "${mode}" = "token" ]; then
    start_argo_token_tunnel "${tag}"
  else
    start_argo_temp_tunnel "${tag}" || return 1
  fi
}
...
restart_all_argo_nodes() {
  ...
  while IFS= read -r tag; do
    ...
    if [ "$(node_value "$tag" "protocol")" = "vless-argo" ]; then
      start_argo_node "$tag" || true
    fi
  done < <(iter_node_tags)
}

ensure_argo_nodes() {
  local tag protocol pid_file pid
...
create_backup() {
  local backup_path="/tmp/singbox-manager-backup-$(date +%Y%m%d-%H%M%S).tar.gz"
  if [ -d "${BASE_DIR}" ]; then
    tar -czf "${backup_path}" -C /usr/local/etc singbox-manager 2>/dev/null || true
    if [ -f "${backup_path}" ]; then
      print_info "备份已保存到：${backup_path}"
    fi
  fi
}
