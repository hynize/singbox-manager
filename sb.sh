...
prompt_domain() {
  ...
  if ! validate_domain "$value"; then
    print_warn "域名格式可能无效：${value}，将继续使用。"
  fi
  printf '%s' "$value"
}
...
render_inbound_for_tag() {
  ...
  jq -n \
    ...
    '$nodes[0][$tag] as $n | $secrets[0][$tag] as $s |
      $n.protocol as $proto |
      if $proto == "vless-reality" then
        { type: "vless", tag: $tag, listen: "::", listen_port: $n.port,
          users: [{ name: $n.name, uuid: $s.uuid, flow: "xtls-rprx-vision" }],
          tls: { enabled: true, server_name: $n.reality_server,
            reality: { enabled: true,
              handshake: { server: $n.reality_server, server_port: 443 },
              private_key: $s.private_key, short_id: [$n.short_id] } } }
      elif $proto == "vless-ws-tls" then
        { type: "vless", tag: $tag, listen: "::", listen_port: $n.port,
          users: [{ name: $n.name, uuid: $s.uuid }],
          tls: { enabled: true,
            certificate_path: $n.certificate_path, key_path: $n.key_path },
          transport: { type: "ws", path: $n.ws_path } }
      elif $proto == "anytls" then
        { type: "anytls", tag: $tag, listen: "::", listen_port: $n.port,
          users: [{ name: $n.name, password: $s.password }],
          tls: { enabled: true,
            certificate_path: $n.certificate_path, key_path: $n.key_path } }
      elif $proto == "vless-argo" then
        { type: "vless", tag: $tag, listen: "127.0.0.1", listen_port: $n.port,
          users: [{ name: $n.name, uuid: $s.uuid }],
          transport: { type: "ws", path: $n.ws_path } }
      elif $proto == "tuic-v5" then
        { type: "tuic", tag: $tag, listen: "::", listen_port: $n.port,
          users: [{ name: $n.name, uuid: $s.uuid, password: $s.password }],
          congestion_control: "bbr", zero_rtt_handshake: false, heartbeat: "10s",
          tls: { enabled: true, alpn: ["h3"],
            certificate_path: $n.certificate_path, key_path: $n.key_path } }
      elif $proto == "hy2" then
        { type: "hysteria2", tag: $tag, listen: "::", listen_port: $n.port,
          users: [{ name: $n.name, password: $s.password }],
          up_mbps: ($n.up_mbps // 200), down_mbps: ($n.down_mbps // 200),
          tls: { enabled: true, alpn: ["h3"],
            certificate_path: $n.certificate_path, key_path: $n.key_path } }
      elif $proto == "socks5" then
        { type: "socks", tag: $tag, listen: "::", listen_port: $n.port,
          users: [{ username: $n.username, password: $s.password }] }
      else empty end'
}
...
add_vless_reality() {
  ...
  reality_server="$(prompt_domain "Reality 域名" "${DEFAULT_REALITY_SERVER}")"
...
add_vless_ws_tls() {
  ...
  uuid="$(prompt_optional_value "UUID（留空自动生成）")"
  uuid="${uuid:-$(generate_uuid)}"
  preferred_domain="$(prompt_domain "优选域名" "${DEFAULT_CDN_DOMAIN}")"
  host_domain="$(prompt_domain "Host/SNI 域名" "${DEFAULT_TLS_SERVER}")"
...
add_anytls() {
  ...
  tls_server="$(prompt_domain "SNI 域名" "${DEFAULT_TLS_SERVER}")"
...
delete_node() {
  ...
  if [ "${protocol}" = "vless-argo" ]; then
    stop_argo_node "$tag"
    rm -f "${BASE_DIR}/logs/${tag}.cloudflared.log"
  fi
...
update_script() {
  ...
  if [ -f "${INSTALL_BIN}" ]; then
    cp "${INSTALL_BIN}" "${backup_tmp}/sbm" 2>/dev/null || true
  fi
  if [ -d "${LIB_DIR}" ]; then
    cp -r "${LIB_DIR}" "${backup_tmp}/lib" 2>/dev/null || true
  fi
  if ! install_release_bundle "${latest_tag}"; then
    print_warn "更新失败，尝试恢复..."
    [ -f "${backup_tmp}/sbm" ] && cp "${backup_tmp}/sbm" "${INSTALL_BIN}" 2>/dev/null || true
    [ -d "${backup_tmp}/lib" ] && cp -r "${backup_tmp}/lib/." "${LIB_DIR}/" 2>/dev/null || true
    rm -rf "${backup_tmp}"
    release_lock
    fatal "项目文件更新失败，已恢复原始文件。"
  fi
  rm -rf "${backup_tmp}"
  sanitize_permissions
  release_lock
